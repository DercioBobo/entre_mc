...
def atualizar_atrasos():
	"""Corre diariamente. Para cada Pedido de Crédito em aberto:
	...
	for nome_pedido in frappe.get_all(
		"Pedido de Crédito", filters={"status": ["in", ESTADOS_EM_ABERTO]}, pluck="name"
	):
		_atualizar_pedido(nome_pedido, settings, hoje)


def _atualizar_pedido(nome_pedido, settings, hoje):
	pedido = frappe.get_doc("Pedido de Crédito", nome_pedido)
